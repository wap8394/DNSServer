...
def encrypt_with_aes(input_string, password, salt):
    salty = 'Tandon'
    key = generate_aes_key('wap8394@nyu.edu', salty.encode('utf-8'))
    f = Fernet(key)
    secretdata = 'AlwaysWatching'
    encrypted_data = f.encrypt(secretdata.encode('utf-8'))  # call the Fernet encrypt method
...
salty = 'Tandon'
salt = salty.encode('utf-8')  # Remember it should be a byte-object
password = 'wap8394@nyu.edu'
...
'nyu.edu.': {
        ...
        dns.rdatatype.TXT: ('put encrypted data here'),
    }
...
def run_dns_server():
    ...
    while True:
        try:
            ...
            request = dns.message.from_wire(53,dns_records) ##### MIGHT BE WRONG COME BACK
